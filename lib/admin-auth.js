...
export async function requireAdmin(request, permission) {
  ...
  let r;
  try {
    r = await pool.query('SELECT id, username, role, permissions, active, deleted_at FROM admins WHERE id = $1', [tok.adminId]);
  } catch {
  ...
  const a = r.rows[0];
  // A soft-deleted admin (deleted_at set) is treated exactly like a suspended
  // one — the token stops working immediately, but the row is kept for history.
  if (a.deleted_at) return { ok: false, status: 403, error: 'Your admin account no longer exists.' };
...
// Convenience permission check against an already-resolved admin object
// (the one returned by requireAdmin). Super admins always pass.
export function hasPerm(admin, permission) {
